...
FER_WEIGHTS = MODELS_DIR / "fer_model.pt"          # you will place final face model here
...
_maybe_div100(songs_df, ["danceability","valence","energy","acousticness","instrumentalness","liveness","speechiness"])


# -------------------------
# Load FER model (placeholder-safe)
# IMPORTANT: This must match the model you trained.
# We'll update FER_ARCH and FER_IMG_SIZE once your best model is finalized.
# -------------------------
device = "cpu"
FER_ARCH = "swin_tiny_patch4_window7_224"
FER_IMG_SIZE = 224

fer_model = timm.create_model(FER_ARCH, pretrained=False, num_classes=len(FER_CLASSES))

if FER_WEIGHTS.exists():
    fer_model.load_state_dict(torch.load(FER_WEIGHTS, map_location=device))
    print(f"✅ Loaded FER weights from {FER_WEIGHTS}")
else:
    print("⚠️ FER weights not found yet. Place models/fer_model.pt to enable emotion prediction.")
...
def predict_emotion(pil_img: Image.Image) -> Dict[str, Any]:
    if not FER_WEIGHTS.exists():
        # fallback if no FER model yet
        return {"emotion": "neutral", "probs": {c: (1.0 if c=="neutral" else 0.0) for c in FER_CLASSES}}
